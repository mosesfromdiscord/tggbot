...
client.on('message', message => {
    ...
    if(message.author.bot) {
        if(message.embeds.length === 1 && message.embeds[0].description.startsWith('React')) {
            message.react(':ticketreact:720350170520485979>')
            .then(msgReaction => console.log('Reacted.'))
            .catch(err => console.log(err));
        }
        if(message.embeds.length === 1 && message.embeds[0].title === 'Ticket Support') {
            message.react(':checkreact:720350170520485979>')
            .then(reaction => console.log("Reacted with " + :envelope_with_arrow: ))
            .catch(err => console.log(err));
        }
    };
    /**
     ...
     * get rid of it.
     */
    if(message.content.toLowerCase() === '?createticket' && message.channel.id === '720350170520485979>') {
